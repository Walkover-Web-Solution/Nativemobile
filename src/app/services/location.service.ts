...
export class LocationService {
    private GoogleApiURL = 'https://maps.googleapis.com/maps/api/geocode/json?key=AIzaSyCaphDTQJXyr1lhnaXP_nm7a5dqgr5KVJU';
    private _: any;
    ...
    public GetCity(location: GeoLocationSearch) {
        ...
        return this._http.get(this.GoogleApiURL + '&' + query, {responseType: 'json'}).pipe(
            map((res) => {
                const r = res as any;
                const data = r.results.filter((i) => _.includes(i.types, 'locality'));
                return data;
            }));
    }
}
