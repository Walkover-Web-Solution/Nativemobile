    ...
    constructor(attrs?: any) {
        ...
        if (attrs) {
            ...
            if (attrs.addresses.length > 0) {
                const str = _.isNull(attrs.addresses[0].address) ? '' : attrs.addresses[0].address;
                // set billing
...
export class SalesTransactionItemClass extends ICommonItemOfTransaction {
    ...
    public setAmount(entry: SalesEntryClass) {
        ...
        setTimeout(() => {
            ...
            const tax = this.getTotalTaxOfEntry(entry.taxes);
    ...
    public getTransactionTotal(tax: number, entry: SalesEntryClass): number {
        ...
        if (tax > 0) {
            ...
            const b = _.cloneDeep(this.getTaxableValue(entry));
            count = a + b;
        } else {
